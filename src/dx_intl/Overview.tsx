...
import { DxIntlBaseRatingDocument, Dx_Intl_Base_Rating } from '../generated/graphql'
import { QueryResult } from '../QueryResult'

const Overview: FunctionComponent = () => {
  const [baseRatingResult] = useQuery({ query: DxIntlBaseRatingDocument })
  const baseRatingAccumulated = (baseRatingResult.data?.dx_intl_base_rating ?? []).reduce<
  Array<Pick<Dx_Intl_Base_Rating, 'range' | 'count'> & { accumulated: number }>
  >((accr, curr) =>
    [...accr, {
      ...curr,
  ...
  return <Container component='main'>
    ...
    <Typography variant='body2'>只計算公開成績單。已排除段位加成。</Typography>
