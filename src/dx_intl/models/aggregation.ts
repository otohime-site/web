...
export const getMojibake = (
  entry: ReturnType<typeof flatSongsResult>[number] & { score?: number },
): string | undefined => {
  /*
  Like what's done in the game, the following string is generated using
  the following JavaScript snippets, which will create garbled title
  by representing UTF-8 bytes in Shift-JIS.

  const getGarbledText = (t) => {
    const encoder = new TextEncoder()
    const decoder = new TextDecoder("shift-jis")
    return decoder.decode(encoder.encode(t))
  }

  In the official song JSON, `title_kana` will not be Japanese if the title
  is not in Japanese. So the song names are traslated to Katakana before generation.
  */

  // Only enable this when nobody had reached the song
  if (entry.score || (entry.play && entry.play > 0)) {
    return undefined
  }
  if (entry.title == "プリズム△▽リズム" && !entry.deluxe) {
    return "繝励Μ繧ｺ繝笆ｳ笆ｽ繝ｪ繧ｺ繝"
  }
  if (entry.title == "Believe the Rainbow" && entry.deluxe) {
    // ビリーブ・ザ・レインボー
    return "繝薙Μ繝ｼ繝悶繧ｶ繝ｻ繝ｬ繧､繝ｳ繝懊"
  }
  if (entry.title === "AFTER PANDORA") {
    // アフター・パンドラ
    return "繧｢繝輔ち繝ｼ繝ｻ繝代Φ繝峨Λ"
  }
  if (entry.title === "Xaleid◆scopiX") {
    // カレイド・スコーピクス
    return "繧ｫ繝ｬ繧､繝峨繧ｹ繧ｳ繝ｼ繝斐け繧ｹ"
  }
  if (entry.title === "Ref:rain (for 7th Heaven)") {
    // リフレイン（フォー・セブンス・ヘブン）
    return "繝ｪ繝輔Ξ繧､繝ｳ医ヵ繧ｩ繝ｼ繝ｻ繧ｻ繝悶Φ繧ｹ繝ｻ繝倥ヶ繝ｳ"
  }
  return undefined
}
